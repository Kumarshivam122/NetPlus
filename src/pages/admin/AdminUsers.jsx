        ...
        <div className="modal-body" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1.25rem' }}>
          ...
          <div style={{ gridColumn:'span 2' }}>
            ...
            <div style={{ background:'var(--gray-50)', borderRadius:'var(--radius-md)', padding:'.85rem', fontSize:'.88rem', color:'var(--gray-600)', lineHeight:1.6 }}>
              {u.storeAddress}, {u.city}, {u.state} – {u.pincode}
            </div>
          </div>
          {/* Documents */}
          <div style={{ gridColumn:'span 2' }}>
...
export default function AdminUsers() {
  ...
  const reload = useCallback(async () => {
    ...
    // Only display retailers in the manage retailers tab
    const retailers = (list || []).filter(u => u.role !== 'admin');
    setUsers(retailers);
  }, []);
  ...
  return (
    <div className="portal-layout" id="admin-users">
      <PortalSidebar title="Manage Retailers" />
      <main className="portal-main">
        <div className="portal-topbar">
          <div>
            <h4 style={{ color:'var(--navy)', fontFamily:'var(--font-display)' }}>Manage Retailers</h4>
            <p style={{ fontSize:'.78rem', color:'var(--gray-400)' }}>{users.length} total registered retailers</p>
          </div>
        </div>
